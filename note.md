...
## 闭包
1. FnOnce 所有闭包都实现了 FnOnce, 意味着至少可以被调用一次。允许闭包消耗它从环境中捕获的变量，此时闭包只能被调用一次
2. FnMut 闭包不消耗变量，实现了 FnMut， 意味着闭包可以被多次调用，且可修改环境变量
3. Fn 闭包不修改环境变量，实现了 Fn， 意味着闭包可以被多次调用，且不改变环境变量

Fn -> FnMut -> FnOnce  
1. 如果实现了Fn，意味着同时也实现了 FnMut FnOnce， 具有Fn特质的闭包满足在 FnMut 和 FnOnce 环境中使用的条件和要求
2. 如果实现了FnMut, 意味着同时也实现了 FnOnce，具有 FnMut特质的闭包满足在 FnOnce 环境中使用的条件和要求
